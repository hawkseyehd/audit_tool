...
function result(job: AuditJobRecord): AuditJobListResult {
  return { items: [job], page: 1, pageSize: 25, total: 1 };
}
...
describe("AuditJobs", () => {
  it("shows client, stage, progress, warnings, and cancels active work", async () => {
    ...
    installApi({ cancelAuditJob, listAuditJobs, retryAuditJob: vi.fn() });
  ...
  it("keeps failed jobs actionable with retry and a clear failure reason", async () => {
    ...
    installApi({ cancelAuditJob: vi.fn(), listAuditJobs, retryAuditJob });
    const user = userEvent.setup();
